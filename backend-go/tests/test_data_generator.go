...
func TestDataGenerator() {
	// 连接测试数据库
	db, err := gorm.Open(postgres.Open("postgres://gymates:gymates123@localhost:5432/gymates?sslmode=disable"), &gorm.Config{})
	if err != nil {
...
func generateFollows(db *gorm.DB, users []models.User) []models.Follow {
	follows := []models.Follow{
		{
			FollowerID:  users[0].UID,
			FollowingID: users[1].UID,
		},
		{
			FollowerID:  users[0].UID,
			FollowingID: users[2].UID,
		},
		{
			FollowerID:  users[1].UID,
			FollowingID: users[2].UID,
		},
	}
